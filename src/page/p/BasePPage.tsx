...
const BasePPage: React.FC<BasePPageProps> = ({apiMethod, modules}) => {
    ...
    const update = () => {
        ...
        const b = Polynomial.toString(p2);
    ...
    const [p2, __setP2] = useState([['1', '1']]);
    const handleChangeP2Numerator = (event: React.ChangeEvent<HTMLInputElement>) => {
        const val = event.target.value;
        const id = parseInt(event.target.id.substr(3));
    ...
    const handleChangeP2Denominator = (event: React.ChangeEvent<HTMLInputElement>) => {
        const val = event.target.value;
        const id = parseInt(event.target.id.substr(3));
    ...
    const p2Inputs = [];
    for(const [_i, _val] of Object.entries(p2)){
        _i !== "0" && p2Inputs.push(
            <AddIcon style={{marginTop: 28}} />
        );
        p2Inputs.push(
            <Grid item>
                <TextField size="medium" style={inputStyles} id={"p2_"+_i} value={_val[0]} onChange={handleChangeP2Numerator} type="text" variant="outlined" />
                {_i !== "0" &&
                <span style={xStyles}>x<sup>{_i}</sup></span>
                }

                <hr />
                <TextField size="medium" style={inputStyles} id={"p2d"+_i} value={_val[1]} onChange={handleChangeP2Denominator} type="text" variant="outlined" />
    ...
    return (
        <form noValidate autoComplete="off">
            ...
            <Grid container spacing={3} alignItems="flex-start">
                ...
                <Grid item>
                    <Card>
                        <IconButton color="primary" style={buttonStyles} size="medium" onClick={addP2Element}><AddIcon /></IconButton>
                        <IconButton style={buttonStyles} size="medium" onClick={removeP2Element}><RemoveIcon /></IconButton>
                    </Card>
                </Grid>
            </Grid>
            <br />
            <Card>
                <CardContent>
                    ...
                    {/*<Typography variant="h6" color="textPrimary" style={{ wordWrap: 'break-word' }}>{output}</Typography>*/}
